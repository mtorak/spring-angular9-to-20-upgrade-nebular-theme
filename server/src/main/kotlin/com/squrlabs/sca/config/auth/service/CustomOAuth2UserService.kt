...
@Service
class CustomOAuth2UserService(@Autowired private val userRepository: UserRepository) :
  ...
  override fun loadUser(oAuth2UserRequest: OAuth2UserRequest): OAuth2User {
    ...
    return try {
      ...
      throw ex
        ...
        ex:
            Exception) { // Throwing an instance of AuthenticationException will trigger the
  ...
  private fun processOAuth2User(
    ...
    var user: UserModel? =
        userRepository.findByEmail(oAuth2UserInfo.getEmail()).map(UserEntityMapper::to).toNullable()
    ...
    } ?: run { user = registerNewUser(oAuth2UserRequest, oAuth2UserInfo) }
    return UserPrincipal.Companion.create(
        user!!, oAuth2User.attributes, rolesToAuthority(user!!.roles))
  }
