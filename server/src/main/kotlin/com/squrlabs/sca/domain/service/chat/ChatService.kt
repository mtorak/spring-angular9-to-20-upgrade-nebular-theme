...
class ChatServiceImpl(
  ...
  override fun newConversation(user: UserPrincipal, email: String): FriendProfileModel {
    userRepository.findByEmail(email).toNullable()?.let { friend ->
      chatRepository
          ...
          ?: run {
            val newCov =
                this.chatRepository.save(ChatEntity(null, user.id, friend.id, "", Date(), Date()))
            simpMessagingTemplate.convertAndSend(
                "/notifications/${friend.id}",
                SocketModel(
                    SocketType.USER_CONVERSATION_ADDED,
                    FriendProfileModel(
                        newCov.id!!, user.mEmail, user.name, user.mImgUrl, newCov.blockedBy)))
            return FriendProfileModel(
                newCov.id, friend.email, friend.name, friend.imgUrl, newCov.blockedBy)
          }
    } ?: run { throw ResourceNotFoundException("User", "email", email) }
  }
  ...
  override fun getConversations(id: String, date: Date): List<FriendProfileModel> {
    ...
    chatRepository.findAllByUser1OrUser2AndUpdatedAtAfter(id, id, date).map {
      if (it.user1 == id) friends[it.user2] = it
      if (it.user2 == id) friends[it.user1] = it
    }
  ...
  override fun blockConversation(id: String, user: UserPrincipal): FriendProfileModel {
    chatRepository.findById(id).toNullable()?.let {
      if ((it.user1 == user.id || it.user2 == user.id) && it.blockedBy == "") {
        ...
        val friend =
            userRepository.findById(if (it.user1 == user.id) it.user2 else it.user1).toNullable()!!
        simpMessagingTemplate.convertAndSend(
            "/notifications/${friend.id}",
            SocketModel(
                SocketType.USER_CONVERSATION_UPDATED,
                FriendProfileModel(
                    newCov.id!!, user.mEmail, user.name, user.mImgUrl, newCov.blockedBy)))
        return FriendProfileModel(
            newCov.id, friend.email, friend.name, friend.imgUrl, newCov.blockedBy)
    ...
    } ?: run { throw ResourceNotFoundException("Conversation", "id", id) }
  }
